...
api_key_onecall = os.environ.get('OPENWEATHER_API_SECRET_KEY_ONECALL')
api_key_aqi = os.environ.get('OPENWEATHER_API_SECRET_KEY_AQI')
...
def call_api(unit_system, location):
    try:
        r_1 = httpx.get('http://api.openweathermap.org/geo/1.0/direct?q={0}&limit=5&appid={1}'.format(location, api_key_onecall)) 
        ...
        if unit_system == 'imperial':
            r_2 = httpx.get('https://api.openweathermap.org/data/2.5/onecall?lat={0}&lon={1}&exclude=minutely&units={2}&appid={3}'.format(lat, lon, unit_system, api_key_onecall))
            degree_unit = '˚F'
            speed_unit = 'mph'
            pressure_unit  = 'mb'
            ...
            r_2 = httpx.get('https://api.openweathermap.org/data/2.5/onecall?lat={0}&lon={1}&exclude=minutely&units=metric&appid={3}'.format(lat, lon, api_key_onecall))
            degree_unit = '˚C'
            speed_unit = 'km/h'
            pressure_unit = 'mb'
            # next_speed_unit = 'mph'
        ...
        r_3 = httpx.get('http://api.openweathermap.org/data/2.5/air_pollution?lat={0}&lon={1}&appid={2}'.format(lat, lon, api_key_onecall))
